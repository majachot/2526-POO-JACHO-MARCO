...
def mostrar_menu():
    ...
    while True:
        ...
        for key in Parciales:
            print(f"{key} - {Parciales[key]}")
        print("0 - Salir")

        eleccion_Parcial = input("Elige una unidad o '0' para salir: ")
        if eleccion_Parcial == '0':
            print("Saliendo del programa.")
            break
        elif eleccion_Parcial in Parciales:
            mostrar_sub_menu(os.path.join(ruta_base, Parciales[eleccion_Parcial]))
        else:
            print("Opción no válida. Por favor, intenta de nuevo.")

def mostrar_sub_menu(ruta_Parcial):
    sub_carpetas = [f.name for f in os.scandir(ruta_Parcial) if f.is_dir()]
        ...
        if eleccion_carpeta == '0':
            ...
            try:
                ...
                if 0 <= eleccion_carpeta < len(sub_carpetas):
                    mostrar_scripts(os.path.join(ruta_parcial, sub_carpetas[eleccion_carpeta]))
                else:
                    print("Opción no válida. Por favor, intenta de nuevo.")
            except ValueError:
                print("Opción no válida. Por favor, intenta de nuevo.")
